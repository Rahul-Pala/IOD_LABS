...
router.get('/filter', (req, res) => {
    ...
    if (matchingFriends.length > 0) {
        // return valid data when the gender and letter matches 
        res.status(200).json(matchingFriends)
    } else {
        // and an error response when there are no matches
        res.status(404).json({error: "No friends matching gender and matching letter"+filterGender +filterLetter})
    }  
})


// 2. Get information about this request from the headers  //HELP
...
router.get('/:id', (req, res) => {
    ...
    let friend = friends.find(friend =>friend.id == userId )

    // Modify this response with the matched friend, or a 404 if not found
    friend ? res.json({result: 'Finding friend with ID ' + friendId})
...
/*router.put('/:id', (req, res) => {
    ...
    // Check if the friend with the given ID exists
    if (friendIndex !== -1) {
        // Update the friend data with the new data
        friends[friendIndex] = { ...friends[friendIndex], ...updatedFriend };

        // Modify this response to return the updated friend data
        res.json({ result: 'Updated friend with ID ' + friendId, data: friends[friendIndex] });
    } else {
        // Return a 404 error if the friend with the given ID is not found
